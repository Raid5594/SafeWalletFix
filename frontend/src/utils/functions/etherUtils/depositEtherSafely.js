...
const deposit = (amountToDeposit, safetyPubKey, privateKey, web3, multisig, multisigAddress, modal, updateBalances, address) => {

    const priv = Buffer.from(privateKey, 'hex');
    web3.eth.getTransactionCount(address, (err, txCount) => {
        web3.eth.getGasPrice().then((gasPrice) => {
            multisig.methods.deposit(safetyPubKey).estimateGas({gas: gasPrice, from: address, value: amountToDeposit}, function(error, gasAmount) {
